...
local function create_explosion(team, hit_props)
...
---@param explosion_hit_props HitProps
local function create_fist(team, facing, hit_props, explosion_hit_props)
  local spell = Spell.new(team)
    ...
    for x = center_x - 1, center_x + 1 do
      for y = center_y - 1, center_y + 1 do
        if x ~= center_x and y ~= center_y then
          -- must be axis aligned, we want to hit in a plus shape
          goto continue
        end
        ...
        if tile ~= spell_tile then
          local explosion = create_explosion(team, explosion_hit_props)
