...
local function load_area(area_id)
    ...
    if object.name == "Arena" then
      ...
      arena_events:on("eject_player", function(event)
        ...
        if personal_launch_dir == "Up" then
          target_y = top - math.random() * 0.5 - 0.3 - (launch_dist - 1)
        else
          target_y = bottom + math.random() * 0.5 + 0.3 + (launch_dist - 1)
        end

        z = z + launch_elevation
...
scripts:instancer():events():on("area_instanced", function(event)
  load_area(event.area_id)
end)

scripts:instancer():events():on("area_removed", function(event)
  BattleArenas.destroy_arenas_in_area(event.area_id)
end)
